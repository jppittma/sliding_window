function output=sliding_window_analysis (time_series, window, step_size, analysis)
	...
	num_windows = floor( base_num_windows / step_size );
	first_window_index = -floor(length(time_series) / 2 - length(window));
	...
	for i = 2:num_windows 
		current_window_index = first_window_index + step_size * i - 1;
